...
const addWishlistItem = (wishlistItems, productsToAdd) => {
  ...
  if(existingWishlistItem) {
    return wishlistItems.map((cartItem) =>  cartItem.id === productsToAdd.id ? {...cartItem , quantity : cartItem.quantity + 1} : cartItem)}

    //normal return 
    return(
      [...wishlistItems , {...productsToAdd , quantity : 1}]
    )
  }
  const removeWishlistItem = (wishlistItems, productToRemove) => {
    ...
    if(existingWishlistItem === 1) {
      return wishlistItems.filter((cartItem) => cartItem.id !== productToRemove.id)
    }
  
    if(existingWishlistItem) {
    return wishlistItems.map((cartItem) =>  cartItem.id === productToRemove.id ? {...cartItem , quantity : cartItem.quantity - 1} : cartItem)}
  ...
  export const WishlistProvider : React.FC<React.ReactNode> = ({children}) => {
    ...
    return(
      <WishlistContext.Provider value={{wishlistItems,saveWishlistItem, deleteWishlistItem}}>
        {children}
